...
| `ParameterManager.GenerateName` / `SqlBuilderBase.MergeSubqueryParameters` / `WhereClause.And` / `WhereClause.Or` | `Bing.Data.Sql.Tests`; `Bing.Dapper.PostgreSql.Tests`; `Bing.Dapper.Oracle.Tests` | `ParameterManagerTest`; `SqlBuilderTest`; `PostgreSqlBuilderTest`; `OracleBuilderTest` | `GenerateName_WhenGeneratedNameAlreadyExists_ShouldSkipExistingName`; `Join_WhenSubqueryContainsQuotedOrCommentedParameterText_ShouldRenameOnlyParameterToken`; `Join_WhenSubqueryContainsDollarQuotedTextOrSystemVariable_ShouldRenameOnlyParameterToken`; `Join_WhenSubqueryContainsDoubleColonCast_ShouldRenameOnlyParameterToken`; `TestAnd_2`; `TestOr_3`; `TestIn_3`; `Test_Union_1` | Unit; independent New Builder 的参数会在组合时完整合并，名称冲突稳定重命名，重复渲染不覆盖已有参数；token 扫描仅替换 SQL 代码中的参数，不改写字符串、标识符、注释、dollar-quoted 文本、系统变量或类型转换。 |
