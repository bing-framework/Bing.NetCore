...
根 `ISqlQuery` 保存连接、事务和诊断状态。每次调用 `Query()`、`From<TEntity>(alias, schema)`、非泛型 `Sql(...)` 或 `Procedure(...)` 都会创建独立查询描述及其专属 Builder。描述遵循 Draft、Frozen、Executing、Completed 生命周期；终结执行使用冻结计划，流式执行在枚举结束或 Dispose 后释放租约。
...
根 `ISqlQuery` 保存连接、事务和诊断状态。需要构建并执行单个查询时，使用 `Query()`、非泛型 `From<TEntity>(alias, schema)`、`Sql(...)` 或 `Procedure(...)` 创建独立查询描述；每个描述都有独立 Builder，执行时仍复用根查询的连接、事务、诊断和 Trace 上下文。

```csharp
var fluent = sqlQuery.Query();
var text = sqlQuery.Sql("Select Id,OrderNo From Orders Where Id=@Id", new { Id = orderId });
```

- `Query()`：创建结果类型由终结方法选择的字符串 Fluent 描述。
...
2. **每次操作创建独立描述**：查询描述在 Draft 阶段可配置，冻结后只能重复执行；需要构造另一条 SQL 时创建新的 `Query()`、非泛型 `From<TEntity>(alias, schema)` 或非泛型原生 SQL 描述。
