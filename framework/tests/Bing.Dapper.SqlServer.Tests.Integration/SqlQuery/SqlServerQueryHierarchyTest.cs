...
/// <summary>
/// SQL Server 树状表真实执行集成测试。
/// </summary>
[Collection(SqlServerIntegrationDatabaseCollection.Name)]
public sealed class SqlServerQueryHierarchyTest : IAsyncLifetime
{
    /// <summary>
    ...
    /// <summary>
    /// 初始化一个<see cref="SqlServerQueryHierarchyTest"/>类型的实例。
    /// </summary>
    /// <param name="fixture">SQL Server 集成测试数据库固定装置。</param>
    public SqlServerQueryHierarchyTest(SqlServerIntegrationDatabaseFixture fixture) => _fixture = fixture;
    ...
    /// <summary>
    /// 测试目的：SQL Server 邻接表递归 CTE 应使用 With 关键字并通过自关联返回完整层级和稳定深度排序。
    /// </summary>
    [IntegrationFact("SqlServer")]
    [Trait("Category", "Integration")]
    [Trait("Database", "SqlServer")]
    public async Task SqlServer_RecursiveCte_WhenQueryingHierarchy_ShouldReturnOrderedTree()
    {
        // Arrange
        await SeedHierarchyNodesAsync();
        using var query = _fixture.CreateQuery();
        var tree = query.Sql<HierarchyNode>().AppendSelect("Id,ParentId,Name,0 As Depth")
    ...
    private async Task SeedHierarchyNodesAsync()
    {
                 ...
                 })
        {
            await executor.ExecuteSqlAsync(
                "Insert Into dbo.BingSqlHierarchyIntegration(Id,ParentId,Name) Values(@id,@parentId,@name)", node);
        }
    }

    /// <summary>
    /// SQL Server 邻接表递归查询投影。
    /// </summary>
