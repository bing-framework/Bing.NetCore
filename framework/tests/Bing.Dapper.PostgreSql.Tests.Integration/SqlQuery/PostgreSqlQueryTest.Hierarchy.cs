...
/// <summary>
/// PostgreSQL SQL 查询对象树状表真实执行集成测试。
/// </summary>
public sealed partial class PostgreSqlQueryTest
{
    /// <summary>
    /// 测试目的：PostgreSQL 邻接表递归 CTE 应使用 With Recursive 并返回完整层级、父子关系和稳定深度排序。
    /// </summary>
    [IntegrationFact("PostgreSql")]
    public async Task PostgreSql_RecursiveCte_WhenQueryingHierarchy_ShouldReturnOrderedTree()
    {
        // Arrange
        await SeedHierarchyNodesAsync();
        using var query = _fixture.CreateQuery();
        var tree = query.Query().AppendSelect("id As \"Id\",parent_id As \"ParentId\",name As \"Name\",0 As \"Depth\"")
            .From("public.integration_hierarchy_nodes").IsNull("parent_id")
            .UnionAll(query.Query().AppendSelect("n.id As \"Id\",n.parent_id As \"ParentId\",n.name As \"Name\",t.\"Depth\" + 1 As \"Depth\"")
    ...
    private async Task SeedHierarchyNodesAsync()
    {
                 ...
                 })
        {
            await executor.ExecuteSqlAsync(
                "Insert Into public.integration_hierarchy_nodes(id,parent_id,name) Values(@id,@parentId,@name)", node);
        }
    }

    /// <summary>
    /// PostgreSQL 邻接表递归查询投影。
    /// </summary>
