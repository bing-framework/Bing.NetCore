...
public sealed class SqliteMutationExecutionIntegrationTest : IAsyncLifetime
{
    ...
    public async Task DeleteBatch_WhenCombinedConcurrencyCommandMatchesAllRows_ShouldDeleteAllRows()
    {
        ...
        using (var query = _fixture.CreateQuery())
            first.Id = query.Select("Id").From("samples").Where("Name", first.Name).ExecuteScalar<int>();
        using (var query = _fixture.CreateQuery())
            second.Id = query.Select("Id").From("samples").Where("Name", second.Name).ExecuteScalar<int>();
    ...
    public async Task Execute_WhenUnifiedMutationBuildersAreConfigured_ShouldExecuteCrud()
    {
        ...
        var insertSelect = executor.GetBuilder()
            .InsertInto("samples")
            .Columns("Name")
            .Select("Name")
            .From("Orders")
        ...
        // Act
        var inserted = await executor.ExecuteAsync(insertSelect);
    ...
    public async Task ExecuteQueryAsync_WhenInsertValuesReturningIsConfigured_ShouldMaterializeRows()
    {
        ...
        // Act
        var rows = (await executor.ExecuteQueryAsync<SqliteReturningRow>()).OrderBy(row => row.Id).ToArray();
    ...
    public async Task ExecuteQueryAsync_WhenInsertSelectReturningIsConfigured_ShouldMaterializeRows()
    {
        ...
        // Act
        var row = Assert.Single(await executor.ExecuteQueryAsync<SqliteReturningRow>());
    ...
    /// <summary>
    /// 测试目的：Executor 应在访问数据库前拒绝执行 Select 状态的统一 Builder。
    /// </summary>
    [Fact]
    public async Task ExecuteAsync_WhenBuilderIsSelect_ShouldThrow()
    {
        // Arrange
        using var executor = _fixture.CreateExecutor();
        var builder = executor.GetBuilder().Select("Id").From("samples");

        // Act
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => executor.ExecuteAsync(builder));

        // Assert
        Assert.Equal("ISqlExecutor 不支持执行 Select 状态的 SQL Builder。", exception.Message);
        Assert.Equal(0, await _fixture.CountAsync());
    }
    ...
    /// </summary>
    [Fact]
    public async Task ExecuteScalarAsync_WhenMutationHasNoReturning_ShouldRejectBeforeExecution()
    {
        // Arrange
        using var executor = _fixture.CreateExecutor();
        executor.GetBuilder().DeleteFrom(new SqlTableReference { TableName = "samples" }).AllowAllRows();

        // Act
        var exception = await Assert.ThrowsAsync<InvalidOperationException>(() => executor.ExecuteScalarAsync<int>());
