...
namespace Bing.Application.Services
{
    ...
    public abstract class QueryAppServiceBase<TEntity, TDto, TQueryParameter, TKey> : ApplicationServiceBase, IQueryAppService<TDto, TQueryParameter>
        ...
        where TQueryParameter : IQueryParameter
    {
        ...
        private IQueryable<TEntity> Filter(IQueryBase<TEntity> query) => IsTracking ? QueryableExtensions.Where(_store.Find(), query) : _store.FindAsNoTracking().Where(query);
