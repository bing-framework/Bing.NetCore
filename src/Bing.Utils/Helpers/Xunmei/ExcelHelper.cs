...
namespace Xunmei.CMS.Utilities
{
    ...
    public static class ExcelHelper
    {
        ...
        public static byte[] DownloadExcel<T>(List<T> varlist)
        {
        ...
        /// <returns>System.Byte[][].</returns>
        public static byte[] DownloadExcel(DataSet dataSource)
        {
            return DownloadExcel(dataSource, null);
        }
        ...
        /// <param name="dataValidity">The data validity.</param>
        /// <returns>System.Byte[][].</returns>
        public static byte[] DownloadExcel2007(DataSet dataSource, List<ExcelColumnTitleModel> columnTitles, string titleName, string templatePath, bool createHeader, string sheetName, int startRowNum, byte[] dataValidity)
        {
            //如果存在数据有效性的数据，直接处理数据有效性的内存数据
            var workbook = dataValidity != null ? GetTemplateWorkbook2007(dataValidity) : GetTemplateWorkbook2007(templatePath);
            bool defaultStyle = (workbook == null);
            if (dataSource != null && dataSource.Tables.Count > 0)
            {
                int tableCount = 1;
                ...
                foreach (DataTable dataTable in dataSource.Tables)
                {
                    ...
                    //设置标题
                    Title2007(workbook, sheet, titleName, dataTable.Columns.Count, defaultStyle);

                    //列头的处理
                    if (createHeader)
                        HeaderCell2007(workbook, sheet, columnTitles, defaultStyle);
                    //填充内容
                    FillExcel2007(workbook, dataTable, sheet, defaultStyle, startRowNum);

                    tableCount++;
                }
                //返回文件的二进制
                return Download2007(workbook);
            }
            return null;
        }
        ...
        /// <param name="dataValidity">The data validity.</param>
        /// <returns>System.Byte[][].</returns>
        public static byte[] DownloadExcel2007(DataSet dataSource, List<string> columnTitles, string titleName, string templatePath, bool createHeader, string sheetName, int startRowNum, byte[] dataValidity)
        {
            //如果存在数据有效性的数据，直接处理数据有效性的内存数据
            var workbook = dataValidity != null ? GetTemplateWorkbook2007(dataValidity) : GetTemplateWorkbook2007(templatePath);
            bool defaultStyle = (workbook == null);
            if (dataSource != null && dataSource.Tables.Count > 0)
            {
                ...
                foreach (DataTable dataTable in dataSource.Tables)
                {
                    ...
                    //设置标题
                    Title2007(workbook, sheet, titleName, columnTitles.Count, defaultStyle);
        ...
        private static HSSFWorkbook GetTemplateWorkbook(Byte[] templateByte)
        {
            ...
            if (templateByte.Length < 1) return workbook;
        ...
        {
            HSSFWorkbook workbook = null;
            if (string.IsNullOrEmpty(templatePath)) return workbook;
            using (FileStream templateFs = new FileStream(templatePath, FileMode.Open, FileAccess.Read))
            {
        ...
        private static void Title(HSSFWorkbook workbook, ISheet sheet, string title, int columnNum, bool defaultStyle = true)
        {
            if (!string.IsNullOrEmpty(title))
            {
        ...
        /// <param name="defaultStyle">if set to <c>true</c> [default style].</param>
        private static void HeaderCell(HSSFWorkbook workbook, ISheet sheet, List<string> columnTitles, bool defaultStyle = true)
        {
            //如果是默认则创建
        ...
        private static void FillExcel(HSSFWorkbook workbook, DataTable dataTable, ISheet sheet, bool defaultStyle = true, int startRowNum = 2)
        {
        ...
        private static ICellStyle[] SetWorkbookStyle(HSSFWorkbook workbook, IRow row, bool defaultStyle = true, int startRowNum = 2)
        {
        ...
        /// <param name="templateByte">The template byte.</param>
        /// <returns>HSSFWorkbook.</returns>
        private static XSSFWorkbook GetTemplateWorkbook2007(Byte[] templateByte)
        {
        ...
        /// <param name="templatePath">The template path.</param>
        /// <returns>HSSFWorkbook.</returns>
        private static XSSFWorkbook GetTemplateWorkbook2007(string templatePath)
        {
            XSSFWorkbook workbook = null;
            if (string.IsNullOrEmpty(templatePath)) return workbook;
            using (FileStream templateFs = new FileStream(templatePath, FileMode.Open, FileAccess.Read))
        ...
        /// <param name="columnNum">The column number.</param>
        /// <param name="defaultStyle">if set to <c>true</c> [default style].</param>
        private static void Title2007(XSSFWorkbook workbook, ISheet sheet, string title, int columnNum, bool defaultStyle = true)
        {
            if (!string.IsNullOrEmpty(title))
            {
                var rootRow = defaultStyle ? sheet.CreateRow(0) : sheet.GetRow(0);
        ...
        /// <param name="columnTitles">The column titles.</param>
        /// <param name="defaultStyle">if set to <c>true</c> [default style].</param>
        private static void HeaderCell2007(XSSFWorkbook workbook, ISheet sheet, List<string> columnTitles, bool defaultStyle = true)
        {
            //如果是默认则创建
            var dataRowFirst = defaultStyle ? sheet.CreateRow(1) : sheet.GetRow(1);
            dataRowFirst = dataRowFirst ?? sheet.CreateRow(1);
            ICellStyle[] styles = SetWorkbookStyle2007(workbook, dataRowFirst, defaultStyle);
            if (defaultStyle && styles != null && styles.Length > 0)
            {
                styles[0].FillPattern = FillPattern.SolidForeground;
                styles[0].FillForegroundColor = HSSFColor.Grey25Percent.Index;
        ...
        /// <param name="columnTitles">The column titles.</param>
        /// <param name="defaultStyle">if set to <c>true</c> [default style].</param>
        private static void HeaderCell2007(XSSFWorkbook workbook, ISheet sheet, List<ExcelColumnTitleModel> columnTitles, bool defaultStyle = true)
        {
            int nowColIndex = 0;
            for (int i = 0; i < columnTitles.Count; i++)
            {
        ...
        /// <param name="defaultStyle">if set to <c>true</c> [default style].</param>
        /// <param name="startRowNum">The start row number.</param>
        private static void FillExcel2007(XSSFWorkbook workbook, DataTable dataTable, ISheet sheet, bool defaultStyle = true, int startRowNum = 2)
        {
            int rowIndex = startRowNum;       // 起始行
            foreach (DataRow row in dataTable.Rows)
            {
                ...
                ICellStyle[] styles = SetWorkbookStyle2007(workbook, dataRow, defaultStyle, startRowNum);
                int columnIndex = 0;
                foreach (DataColumn column in dataTable.Columns)
                {
                    // 列序号赋值
        ...
        /// <param name="workbook">The workbook.</param>
        /// <returns>System.Byte[][].</returns>
        private static byte[] Download2007(XSSFWorkbook workbook)
        {
            using (MemoryStream ms = new MemoryStream())
            {
                workbook.Write(ms);
        ...
        /// <param name="startRowNum">The start row number.</param>
        /// <returns>ICellStyle[][].</returns>
        private static ICellStyle[] SetWorkbookStyle2007(XSSFWorkbook workbook, IRow row, bool defaultStyle = true, int startRowNum = 2)
        {
            List<ICellStyle> styles = new List<ICellStyle>();
            if (defaultStyle)
            {
