...
const ROLE_META = {
  'plan-writer': {
    ...
    codexInstructions:
      '你是 Planner。读取项目 AGENTS.md 和真实源码，生成 plan.md，不修改业务代码。必须读取并遵循 .agents/skills/create-plan/SKILL.md。',
  ...
  'code-reviewer': {
    ...
    codexInstructions:
      '你是独立 Reviewer。必须读取并遵循 .agents/skills/review-code/SKILL.md；不修改业务代码。未解决 MUST_FIX/SHOULD_FIX 时保持 NEEDS_FIX。',
    antigravityInstructions: [
      '你是独立 Reviewer。',
      '必须读取并遵循 .agents/skills/review-code/SKILL.md。',
      '只审查，不修改业务代码。',
...
function writeCodex(workspaceRoot, profileName, roles, dryRun) {
  ...
  for (const [role, cfg] of Object.entries(roles)) {
    const meta = ROLE_META[role];
    const lines = [
      `# Generated by Universal Agent Workflow V4.4 (${profileName})`,
    ...
    lines.push(`# execution_mode = ${tomlString(cfg.executionMode || 'default')}`);
...
function antigravityAgentContent(role, cfg, effortMode, profileName) {
  ...
  lines.push(`执行模式：\`${cfg.executionMode || 'interactive'}\``);
...
function printSummary(profileName, profile, targets, reports, dryRun) {
  ...
  for (const target of targets) {
    ...
    for (const [role, cfg] of Object.entries(report.roles)) {
      ...
      lines.push(`  执行模式：${cfg.executionMode || 'default'}`);
