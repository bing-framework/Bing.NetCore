 ...
 */
export let toNumber = (value, precision?, isTruncate?: boolean): number => {
    if (!isNumber(value))
        return 0;
...
/**
 * 泛型集合转换
 * @param input 输入值，以逗号分隔的元素集合字符串，范例: 1,2
 */
 ...
 * @param target 目标集合
 */
export let concat = <T>(source: T[], target: T[]): T[] => {
 ...
 */
export let truncate = (input: string, length?: number): string => {
 ...
 * @param index 索引
 */
export let insert = (source: any[], item, index?: number): any[] => {
 ...
 * @param name 文件名
 */
export let getExtension = (name: string): string => {
    if (!name)
        return null;
    return `.${name.replace(/.+\./, '')}`;
};
 ...
 * @param value 日期值
 */
export let isBeforeToday = (value: Date): boolean => {
 ...
 * @param value 日期值
 */
export let isBeforeTomorrow = (value: Date): boolean => {
    const date = formatDate(value);
 ...
 * @param name 文件名称
 */
export let isImage = (name: string): boolean => {
